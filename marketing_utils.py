...
def estimate_lead_conversions_kpi(
    ...
    if not required_cols.issubset(data.columns):
        raise ValueError(f"CSV must contain columns {required_cols}, got {set(data.columns)}")
    ...
    if priors is None:
        ...
        priors = {
            ...
            "alpha_conv": 1.0,
            "beta_conv": (1.0 - avg_conv_rate) / avg_conv_rate if avg_conv_rate > 0 else 99.0
        }
    ...
    p_samples = beta.rvs(
        a=alpha_post_p, b=beta_post_p, size=n_sim, random_state=rng
    )
